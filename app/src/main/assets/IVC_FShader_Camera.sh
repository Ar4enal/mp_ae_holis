...
uniform samplerExternalOES sampler2d1;
void main() {
      if(stereoARMode == 2){
         gl_FragColor = texture2D(sampler2d1, vTexCoord);
      }else if(stereoARMode == 3){
         ...
         gl_FragColor = p*vec4(0.0,0.0,0.0,0.5)+(1.0-p)*texture2D(sampler2d1, vec2(x, y));
      }
